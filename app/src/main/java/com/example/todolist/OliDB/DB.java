...
public class DB extends DBM {
     ...
     */
    public static DB initFor(Application app, String dbName, int dbVersion) {
        if (instance == null) {
            instance = new DB(app, dbName, dbVersion);
        }
        return instance;
    }
    ...
    public long insertTask(String taskName, String taskDescription, int completed) {
        ...
        Task task = getNewTask(taskName, taskDescription, completed);
        task.setId((int) id);
        db.close();
     ...
     */
    private Task getNewTask(String taskName, String taskDescription, int completed)
    {
        Task task = new Task();
